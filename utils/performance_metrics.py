...
def confusion_matrix(preds, labels, num_classes):
    ...
    '''
    task = 'binary' if num_classes == 2 else 'multiclass' # ConfusionMatrix also supports multilabel
...
def multiply_diff_mht(acc_eta, hittimes):
    ...
    mht = torch.mean(hittimes, dim=1) if len(hittimes.shape) > 1 else hittimes
        ...
        # to (num thresh,)
    _mht = torch.cat(
        [mht[1:], torch.tensor([time_steps], dtype=torch.float32).to(_device)], 
        dim=0)
    diff_mht = _mht - mht
...
def multiplet_sequential_confmx(logits_concat, labels_concat):
    ...
    """
    logits_concat_shape = logits_concat.shape # (batch, (time_steps - order_sprt), order_sprt + 1, num classes)
    ...
    for iter_idx in range(logits_concat_shape[2]):
        preds_former = torch.argmax(logits_concat_former[:,iter_idx,:], dim=-1).to(torch.int32) # (batch,)
        if iter_idx == 0:
            seqconfmx_mult = confusion_matrix(preds_former, labels_concat, num_classes).to(torch.int32)
            seqconfmx_mult = torch.unsqueeze(seqconfmx_mult, 0)
        else:
            seqconfmx_mult = torch.cat(
                [seqconfmx_mult, torch.unsqueeze(
                    confusion_matrix(preds_former, labels_concat, num_classes).to(torch.int32), 
                    0)],
                dim=0
                )

    # Latter time_steps-order_sprt-1 frames
    logits_concat_latter = logits_concat[:,1:,-1,:] # (batch, time_steps-order_sprt-1, num classes)

    for iter_idx in range(logits_concat_shape[1]-1):
        preds_latter = torch.argmax(logits_concat_latter[:,iter_idx,:], dim=-1).to(torch.int32) # (batch,)
        seqconfmx_mult = torch.cat(
            [seqconfmx_mult, torch.unsqueeze(confusion_matrix(preds_latter, labels_concat, num_classes).to(torch.int32), dim=0)],
            dim=0
            )
...
def average_dicts(dict_list):
    ...
    assert all(d.keys() == keys for d in dict_list), 'All dictionaries must have the same keys.'

    return {key: torch.mean(torch.stack(values))
...
def seqconfmx_to_macro_ave_sns(seqconfmx): 
    ...
    if len(gathered_SNS) % time_steps == 0:
        reshaped_SNS = gathered_SNS.reshape(time_steps, -1)
    else:
        pdb.set_trace()

    macroave_sns = torch.mean(
            reshaped_SNS, 
        dim=1, keepdims=True)
        # (time_steps, 1)
...
def calc_llrs(logits_concat):
    ...
    if order_sprt == 0:
        ...
        for iter_frame in range(time_steps):
            llrs = torch.sum(
                llrs_all_frames[:, :iter_frame+1, :, :], dim=1) 
    ...
    llr_matrix = torch.cat(list_llrs, dim=1) # (batch, time_steps, num cls, num cls)
...
def threshold_generator(llrs, num_thresh, sparsity):
    ...
    thresh = thresh.reshape(num_thresh, 1, 1, 1)
    thresh = thresh.repeat(1, time_steps, num_classes, num_classes).to(_device)
        # (num thresh, num cls, num cls)
    mask = 1 - torch.eye(num_classes).to(_device) # diagonal all-zero, else one 
...
def thresh_truncated_MSPRT(thresh):
    '''
    Args:
    - thresh (TensorFlow tensor): size (NUM_THRESH, TIME_STEP, NUM_CLASSES, NUM_CLASSES)
    '''
    _device = thresh.device
    num_thresh, time_step, num_classes, _ = thresh.shape

    mask = 1 - torch.eye(num_classes).to(_device) # diagonal all-zero, else one 
    last_thresh = torch.zeros((num_thresh, 1, num_classes, num_classes)).to(_device)
    last_thresh += mask * 1e-11

    return torch.cat([thresh[:, :-1, :, :], last_thresh], dim=1)
...
def thresh_sanity_check(thresh_mtx):
    ...
    """
    """ check this works for batch, time_steps, num thresh = 1"""
    num_classes = thresh_mtx.shape[2]
    _device = thresh_mtx.device
    
    for i in range(num_classes):
        if not (thresh_mtx[:, :, i, i] == 0).all():
            raise ValueError(
    ...
    if not (tmp_th > 0).all():
        raise ValueError("The off-diag elements of thresh_mtx must be positive."+
            "\nNow thresh_mtx = {}".format(thresh_mtx)
            )
...
def truncated_MSPRT(llr_mtx, labels_concat, thresh_mtx):
    ...
    if time_steps == 1:
        ...
        # Calc hitting times
        mht = torch.tensor(1., dtype=torch.float32)
        vht = torch.tensor(0., dtype=torch.float32)
