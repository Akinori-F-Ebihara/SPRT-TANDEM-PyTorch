...
from utils.misc import (StopWatch, create_directories_and_log,
...
def float_formatter(variable):
    ...
    if isinstance(variable, float):
        ...
        stripped_variable = variable.replace(".", "").replace(
...
class AvoidForbiddenParamsSampler(optuna.samplers.BaseSampler):
    ...
    def sample_independent(self, study: optuna.study.Study, trial: optuna.trial.FrozenTrial,
        ...
        should_use_random_sampler = self.should_use_random_sampler(
            current_param_set)
...
def run_optuna(objective, device, config):
    ...
    def start_optimization(objective, conf):
        ...
        '''
        num_objectives = len(
            conf.optimization_candidates) if 'all' in conf.optimization_target.lower() else 1
    ...
    def reproduce_trial(objective, conf):
        ...
        for cnt in range(conf.num_trials):
            ...
            logger.info(colored(f'reproducing the trial,\n',
                        'yellow') + f'{trial},from:\n{path_db}')
            pdb.set_trace()
            paramtable = ConsoleTable('Loaded parameters', [
                                      'Parameter name', 'Value'])
            for key, value in trial.params.items():
                key = key.replace('list_', '') if key.startswith(
                    'list_') else key
    ...
    def trial_statistics(study, conf):
        ...
        def print_value_and_params(trial, conf, num_objectives):
            if num_objectives == 1:
                paramtable = ConsoleTable(
                    'Objective', [conf.optimization_target])
                paramtable.add_entry([trial.value])
            else:
                paramtable = ConsoleTable(
                    'Objectives', conf.optimization_candidates)
                paramtable.add_entry(trial.values)
            paramtable.print()
            if 'tuning' in conf.exp_phase:
                paramtable = ConsoleTable(
                    'Best params', ['Parameter name', 'Value'])
                for key, value in trial.params.items():
                    key = key.replace('list_', '') if key.startswith(
                        'list_') else key
                    paramtable.add_entry([key, float_formatter(value)])
        ...
        pruned_trials = len(study.get_trials(
        ...
        if num_objectives == 1:
            ...
            if completed_trials > 1:
                print_importance(
                    optuna.importance.get_param_importances(study), conf)
...
def setup_model_parameter_space(config: Dict[str, Any], model_key: str, model_zoo: List[str]) -> None:
    ...
    config.update(
        {key: value for key, value in config[model_name.upper()].items()})
